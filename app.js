...
window.addEventListener("scroll", () => {
  saluteTextContainer.style.transform = `translateY(${window.scrollY}px)`;
  saluteImgContainer.style.transform = `translate(${window.scrollY * 0.4}px, ${
    window.scrollY * 0.7
  }px)`;
  authorName.style.transform = `translateX(${window.scrollY * 0.2}px)`;
  jobTitleContainers[0].style.backgroundPositionY = `${window.scrollY * 0.5}px`;
  jobTitleContainers[1].style.backgroundPositionY = `${
    -window.scrollY * 0.5
  }px`;
  jobTitles[0].style.transform = `translateX(calc(200vh - ${window.scrollY}px))`;
  jobTitles[1].style.transform = `translateX(calc(-300vh + ${window.scrollY}px))`;
  projectsTitle.style.transform = `translateY(calc(400vh - ${window.scrollY}px))`;
  phones[0].style.transform = `translateX(calc(500vh - ${window.scrollY}px))`;
  laptop.style.transform = `translateX(calc(600vh - ${window.scrollY}px))`;
  phones[1].style.transform = `translateX(calc(750vh - ${window.scrollY}px))`;
});
